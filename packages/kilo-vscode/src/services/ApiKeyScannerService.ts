 ...
 *
 * All disk I/O is async to keep the extension host event loop responsive
 * during onboarding scans.
 */
...
/**
 * Read a UTF-8 file, returning `null` if the file does not exist. All other
 * errors propagate. Callers use the `null` return to mirror the legacy
 * `existsSync(p) ? readFileSync(p) : null` pattern without the extra stat.
 */
...
async function tryReaddir(dirPath: string): Promise<string[] | null> {
  try {
...
export class ApiKeyScannerService {
  ...
  static async scan(): Promise<ScanResult> {
    const [
      ...
      this.scanOpenAI(),
      this.scanElevenLabs(),
      Promise.resolve(this.scanPolly()),
    ])
    return { azure, siliconflow, minimax, github, huggingface, google, openai, elevenlabs, polly }
  }
   ...
   */
  static async getDiscoverySummary(): Promise<Record<string, boolean>> {
    const result = await this.scan()
    return {
  ...
  private static async scanAzure(): Promise<{ apiKey: string | null; region: string | null }> {
    ...
    const fileNames = await tryReaddir(apiDir)
    if (fileNames) {
      const envFiles = fileNames.filter((f) => f.startsWith(".env") || f.endsWith(".env"))

      // Read all candidate files in parallel; preserve original ordering when
      // selecting the first match.
      const contents = await Promise.all(envFiles.map((name) => tryRead(path.join(apiDir, name))))
      ...
      const azureTxt = await tryRead(path.join(apiDir, "env", "Azure.txt"))
      if (azureTxt) {
        const key = this.extractValue(azureTxt, ["AZURE_SPEECH_KEY"])
        const region = this.extractValue(azureTxt, ["AZURE_SPEECH_REGION"])
        if (key) {
          return { apiKey: key, region: region || "westus" }
        }
      }
  ...
  private static async scanSiliconFlow(): Promise<string[]> {
    const keys: string[] = []
    ...
    const contents = await Promise.all(files.map((file) => tryRead(path.join(apiDir, file))))
    for (const content of contents) {
      if (!content) continue
  ...
  private static async scanMinimax(): Promise<string | null> {
    // Check environment
    ...
    for (const file of files) {
      const content = await tryRead(path.join(apiDir, file))
      if (!content) continue
  ...
  private static async scanGithub(): Promise<string | null> {
    // Check environment
  ...
  private static async scanHuggingFace(): Promise<string | null> {
    // Check environment
  ...
  private static async scanGoogle(): Promise<string | null> {
    ...
    const apiDir = API_DOWNLOADS_PATH
    const content = await tryRead(path.join(apiDir, ".env"))
    if (!content) return null
    return this.extractValue(content, ["GOOGLE_API_KEY", "GCLOUD_API_KEY"])
  }

  private static async scanOpenAI(): Promise<string | null> {
  ...
  private static async scanElevenLabs(): Promise<string | null> {
